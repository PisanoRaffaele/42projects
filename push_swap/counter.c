...
/*   Updated: 2024/02/28 13:53:53 by rpisano          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_count_rarb(t_stack *a, t_stack *b, int num, char c)
{
	...
	{
		i = pos_a(a, num);
		if (i < num_index(b, num))
			i = num_index(b, num);
	}
	else
	{
		i = pos_b(b, num);
		if (i < num_index(a, num))
			i = num_index(a, num);
	}
	return (i);
}
...
int	ft_count_rrarrb(t_stack *a, t_stack *b, int num, char c)
{
	...
	if (c == 'b')
	{
		...
		if ((i < (get_size(a) - num_index(a, num))) && num_index(a, num))
			i = get_size(a) - num_index(a, num);
	}
	else
	{
		if (pos_a(a, num))
			i = get_size(a) - pos_a(a, num);
		if ((i < (get_size(b) - num_index(b, num))) && num_index(b, num))
			i = get_size(b) - num_index(b, num);
	}
	return (i);
}
...
// This function calculates which rotation combination
// is the best to sort the stack.
// c is the name of the stack that num is moved to.
